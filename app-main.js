...
function detectApiBase(){
  ...
  var host=(location.hostname||"").toLowerCase();
  if(host==="ads.cepat.top")return "";
  return "https://ads.cepat.top";
}
...
function RegisterPage(props){
  ...
  var st4=useState(""),confirmPass=st4[0],setConfirmPass=st4[1];
  var st5=useState(""),error=st5[0],setError=st5[1];
  ...
  function handleSubmit(e){
    ...
    setLoading(true);
    authReq("/auth/register",{name:name,email:email,password:password})
      .then(function(res){
        setLoading(false);
        if(res.ok){
          saveAuth(res.token,res.user);
...
function AdminUserPanel(props){
  ...
  function loadStats(){
    ...
    authReq("/admin/stats",{},"GET")
      .then(function(res){if(res.ok)setStats(res.stats);});
  }
...
function App(){
  ...
  function loadLiveSnapshot(showMsg){
    if(!HAS_EXPLICIT_APIBASE && (location.hostname||"").toLowerCase()==="ads.cepat.top"){
      setLiveState("local");
